...
func NewTxPool(config TxPoolConfig, chainconfig *params.ChainConfig, eventMux *event.TypeMux, currentStateFn stateFn) *TxPool {
...
func (pool *TxPool) eventLoop() {
	...
	for {
		...
		case ev, ok := <-pool.events.Chan():
			...
			//the event, is caused by during detected chain fork
			//when synchronized with better chain, the tx in old chain
			//should be re-applyed
