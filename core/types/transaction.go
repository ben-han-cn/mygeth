...
func NewTransaction(nonce uint64, to common.Address, amount *big.Int, data []byte) *Transaction {
	return newTransaction(nonce, &to, amount, data)
}

func NewContractCreation(nonce uint64, amount *big.Int, data []byte) *Transaction {
	return newTransaction(nonce, nil, amount, data)
}
...
func (tx *Transaction) BenString() string {
	return fmt.Sprintf("nonce:%v, value:%v", tx.Nonce(), tx.Value())
}
...
type TransactionsByPriceAndNonce struct {
	...
	heads TxByNonce                       // Next transaction for each unique account (price heap)
}
...
func NewMessage(from common.Address, to *common.Address, nonce uint64, amount *big.Int, data []byte, checkNonce bool) Message {
	return Message{
		from:       from,
		to:         to,
		nonce:      nonce,
