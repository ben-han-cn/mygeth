...
func SetupGenesisBlock(db ethdb.Database, genesis *Genesis) (*config.ChainConfig, common.Hash, error) {
	if genesis != nil && genesis.Config == nil {
		return config.AllProtocolChanges, common.Hash{}, errGenesisNoConfig
	}
...
func (g *Genesis) configOrDefault(ghash common.Hash) *config.ChainConfig {
	...
	default:
		return config.AllProtocolChanges
	}
}
...
func (g *Genesis) Commit(db ethdb.Database) (*types.Block, error) {
	...
	conf := g.Config
	if conf == nil {
		conf = config.AllProtocolChanges
	}
	return block, WriteChainConfig(db, block.Hash(), conf)
}
...
func DefaultRinkebyGenesisBlock() *Genesis {
	return &Genesis{
		Config:     config.RinkebyChainConfig,
...
func DevGenesisBlock() *Genesis {
	return &Genesis{
		Config:     config.AllProtocolChanges,
		Nonce:      42,
		GasLimit:   4712388,
